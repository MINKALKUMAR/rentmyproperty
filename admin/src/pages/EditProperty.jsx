...
export default function EditProperty() {
    ...
    const fetchProperty = async () => {
      try {
        ...
        setFormData({
          ...
          type: property.type || '1 RK',
          availability: property.availability || 'FOR ALL',
          furnishing: Array.isArray(property.furnishing) ? property.furnishing : [],
          propertyType: property.propertyType || 'Apartment',
          price: property.price || '',
          rentPeriod: property.rentPeriod || 'per month',
          status: property.status || 'active',
          amenities: Array.isArray(property.amenities) ? property.amenities : [],
          images: property.images || []
        });

        setPreviewMedia(property.images?.map(img => ({
          url: img.url,
          type: img.url.match(/\.(mp4|webm|mov)$/) ? 'video' : 'image'
        })) || []);
  ...
  const removeMedia = (index) => {
    ...
    if (previewMedia[index].preview && previewMedia[index].preview.startsWith('blob:')) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key !== 'images' && key !== 'amenities' && key !== 'furnishing') {
      ...
      // Append only new files that are File objects
  ...
  if (loading && !formData.pid) {
    return <div className="flex justify-center items-center h-64 text-xl">Loading property data...</div>;
  }
          ...
          <div className="bg-blue-600 py-5 sm:py-6 px-5 sm:px-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between">
              <div className="mb-4 sm:mb-0">
                ...
                <p className="text-blue-100 mt-2 text-lg sm:text-xl">Update all required fields</p>
              </div>
              <button
                onClick={() => navigate('/properties')}
